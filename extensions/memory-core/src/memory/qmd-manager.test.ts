...
describe("QmdMemoryManager", () => {
  ...
  it("avoids destructive rebind when qmd only reports collection names", async () => {
    ...
    spawnMock.mockImplementation((_cmd: string, args: string[]) => {
      if (args[0] === "collection" && args[1] === "list") {
        const child = createMockChild({ autoClose: false });
        emitAndClose(child, "stdout", JSON.stringify(["workspace-main", sessionCollectionName]));
        return child;
      }
      return createMockChild();
    });
  ...
  it("warns instead of silently succeeding when add conflict metadata is unavailable", async () => {
    ...
    expectMockMessageNotContains(logWarnMock, "qmd collection remove workspace-legacy");
